...
// map.txt파일을 읽어온 뒤 맵을 초기화하는 함수
void checkMap(int map[][mapX]) {
    ifstream mapFile("map.txt"); // 파일 읽기 모드로 열기

    if (mapFile.is_open()) { // 파일의 오픈 상태 확인
        for (int i = 0; i < mapY; ++i) {
            for (int j = 0; j < mapX; ++j) {
                if (!(mapFile >> map[i][j])) { // 파일에서 값을 읽어온 후 맵 초기화
                    // 파일에서 값이 올바른지 확인
                    cerr << "파일을 읽는 도중 오류가 발생하였습니다. 확인해주세요!" << endl;
                    // 예외 처리 적용 : 파일 읽기 오류 발생 시 프로그램 종료
                    exit(1);
                }
            }
        }
        ...
        // 파일 열기 실패 시 예외 처리
        cerr << "파일을 열 수 없습니다. 파일을 다시 확인해주세요!" << endl;
        // 예외 처리: 파일 열기 실패 시 프로그램 종료
        exit(1);
    }
}