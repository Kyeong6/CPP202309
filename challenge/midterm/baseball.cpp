...
// 3자리 숫자의 자릿수가 모두 다른 지 체크하는 함수 : checkNumber()
...
int main() {
	...
	while (1) {
		...
		while (1) {
			...
			// 숫자를 입력했을 때 중복된 숫자가 있을 경우 문장을 출력하고 다시 입력 받기위한 코드
			if (guessFirst == guessSecond || guessFirst == guessThird || guessSecond == guessThird) {
				cout << "입력된 숫자에 중복된 숫자가 있습니다. 다시 입력하세요." << endl;
				continue;
			}
			// 위에서 작성한 코드를 기반으로 만약 입력한 자릿수의 수자가 모두 다른 경우 same 변수를 true로 활성화시켜 다음 기능 진행
			if (guessFirst != guessSecond && guessFirst != guessThird && guessSecond != guessThird) {
				same = true;
			}
